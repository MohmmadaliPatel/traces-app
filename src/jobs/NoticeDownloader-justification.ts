...
export default class NoticeDownloaderJustification {
  ...
  async getTracesData(company: any) {
    try {
      ...
      // Filter reqList.data.rows for all available Justification Report files
      const allAvailableFiles = (reqList.data.rows || []).filter(
        (row: any) => row.status === "Available" && row.dntype === "Justification Report"
      )

      if (allAvailableFiles.length === 0) {
        this.logger.log("No Justification Report files available")
        return {
          success: false,
          reason: "No Justification Report files available",
      ...
      this.logger.log(`Found ${allAvailableFiles.length} available Justification Report files`)
    ...
    try {
      ...
      if (isManualCaptcha) {
        ...
        try {
          ...
          this.logger.log(
            `Captcha buffer type: ${typeof captchaImageBuffer}, isBuffer: ${Buffer.isBuffer(
              captchaImageBuffer
            )}`
          )
          this.logger.log(
            `Captcha base64 type: ${typeof captchaBase64}, length: ${captchaBase64?.length}`
          )
          this.logger.log(`Captcha base64 preview: ${captchaBase64.substring(0, 50)}...`)
      ...
      // Step 5: Navigate to justrepdwnld.xhtml
      this.logger.log("Navigating to justification report page...")
      ...
      while (hasNextPage) {
        ...
        // Filter available Justification Report files on this page
        const availableFiles = pageRows.filter(
          (row: any) =>
            row.status.toLowerCase() === "available" &&
            row.fileProcessed.toLowerCase() === "justification report"
        )

        this.logger.log(`Found ${availableFiles.length} available Justification Report files on this page`)
      ...
      if (downloadResults.length === 0) {
        this.logger.log("No Justification Report files available to download")
        return {
          success: false,
          reason: "No Justification Report files available",
          company: record.name,
  ...
  async getTracesDatapuppeteer(record: any) {
    ...
    try {
      ...
      if (isManualCaptcha) {
        ...
        this.logger.log(
          `Captcha buffer type: ${typeof captchaImageBuffer2}, isBuffer: ${Buffer.isBuffer(
            captchaImageBuffer2
          )}`
        )
        this.logger.log(
      ...
      // Step 5: Navigate to justrepdwnld.xhtml
      this.logger.log("Navigating to justification report page...")
      await page.goto("https://www.tdscpc.gov.in/app/ded/justrepdwnld.xhtml", {
        waitUntil: "networkidle2",
      })

      // Step 6: Select financial year, quarter, and form type
      this.logger.log("Selecting financial year, quarter, and form type...")
      await page.waitForSelector("#finYr")
      await page.select("#finYr", record.financialYear.toString())
      await page.select("#qrtr", record.quarter == "Q1" ? "3" : record.quarter == "Q2" ? "4" : record.quarter == "Q3" ? "5" : "6")
      await page.select("#frmType", record.formType)
