...
export default class NoticeDownloaderConso {
      ...
      while (hasNextPage) {
        ...
        const availableFiles = pageRows.filter(
          (row: any) =>
            ...
            row.fileProcessed.toLowerCase() === "nsdl conso file"
        )

        this.logger.log(`Found ${availableFiles.length} available NSDL Conso files on this page`)
  ...
  async getTracesDatapuppeteer(record: any) {
    ...
    try {
      ...
      // Step 5: Navigate to nsdlconsofile.xhtml
      this.logger.log("Navigating to consolidated file page...")
      await page.goto("https://www.tdscpc.gov.in/app/ded/nsdlconsofile.xhtml", {
        waitUntil: "networkidle2",
      })
